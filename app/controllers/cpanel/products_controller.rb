...
class Cpanel::ProductsController < Cpanel::ApplicationController
  ...
  private
    ...
    def product_params
      params.require(:product).permit(:type_id, :title, :subtitle, :intro, :discount_score, :stock_count, :delivered_at, :image, :is_discount, :units, :image_cache, :low_price, :origin_price, :discounted_at)
    end
end
