...
module UsersHelper
  ...
  def deliver_select_tag(f)
    ...
    if current_user.apartment_id.present?
      ...
      select_html = f.select :apartment_id, Apartment.opened.map { |a| ["#{a.name}（#{a.address}）", a.id, 'user-apartment-id' => current_user.apartment_id ] }, { prompt: "选择小区", selected: current_user.apartment_id}, { class: "form-control", 'data-current' => current_user.apartment_id }
